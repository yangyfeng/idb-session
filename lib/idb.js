import { promisifyRequest, log, errInfo } from "./utils";
...
class IdbStore {
  ...
  createStore(dbName = this.dbName, storeName = this.storeName) {
    ...
    // onupgradeneeded 版本升级或新建库 触发的事件
    request.onupgradeneeded = (e) => {
      log("版本升级或新建库成功");
      const db = e.target.result;
      const isStoreNameExists = db.objectStoreNames.contains(storeName);
      try {
        !isStoreNameExists && db.createObjectStore(storeName);
      } catch (error) {
        log("创建数据表失败", error.message);
      }
    };
  ...
  keys(customStore) {
    ...
    return customStore("readonly", (store) => {
      ...
      const items = [];
      return this.eachCursor(store, (cursor) => items.push(cursor.key)).then(
        () => items
      );
    });
  }
