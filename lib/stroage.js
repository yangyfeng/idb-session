...
const idbStore = new IdbStore({ dbName: 'ry-index-database', storeName: 'KeyValue' })

// log
const log = (name = '', message) => {
  console.info(`%c ===${name}===`, 'font-size:12px;color:#d3677e;', message)
}
// 转成json
...
// -------------------------返回数据--------------------
const idbSession = {
  // 创建之前的准备（当有两个页面需要加载时，在填充数据时需要调用）
  readyCreate: idbStore.readyCreate,
  // 获取IdbStore => 数据库对象实例
  getIdbStore: () => idbStore
}
